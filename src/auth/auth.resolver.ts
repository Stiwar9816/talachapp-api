...
export class AuthResolver {
  ...
  @Mutation(() => AuthResponde, { name: 'signup', description: 'Create a new user' })
  async signUp(@Args('signupInput') signupInput: SignupInput): Promise<AuthResponde> {
    return this.authService.signup(signupInput)
  }

  @Mutation(() => AuthResponde, { name: 'signin', description: 'User login' })
  async signIn(@Args('signinInput') siginInput: SigninInput): Promise<AuthResponde> {
    return this.authService.signin(siginInput)
  }

  @Query(() => AuthResponde, { name: 'revalidate', description: 'Validates the token of the logged in user' })
